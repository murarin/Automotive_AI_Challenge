...
from src.dataset import CocoDataset, LGSVLDataset, Resizer, Normalizer, Augmenter, collater
...
def train(opt):
    ...
    training_set = LGSVLDataset(root=opt.data_path, image_sets="train",
                               transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]))
    training_generator = DataLoader(training_set, **training_params)

    test_set = LGSVLDataset(root=opt.data_path, image_sets="test",
    ...
    for epoch in range(opt.num_epochs):
        ...
        if epoch % opt.test_interval == 0:
            ...
            if loss + opt.es_min_delta < best_loss:
                best_loss = loss
                best_epoch = epoch
                torch.save(model, os.path.join(opt.saved_path, "best_{}.pth".format(epoch)))
