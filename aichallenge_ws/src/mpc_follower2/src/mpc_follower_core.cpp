...
#include "mpc_follower2/mpc_follower_core.h"
    ...
    : nh_(""), pnh_("~"), my_position_ok_(false), my_velocity_ok_(false), my_steering_ok_(false)
{
  ...
  pnh_.param("in_vehicle_status_name", in_vehicle_status, std::string("odom"));
  ...
  sub_vehicle_status_ = nh_.subscribe("/odom", 1, &MPCFollower::callbackVehicleStatus, this); /////
...
bool MPCFollower::calculateMPC(double &vel_cmd, double &acc_cmd, double &steer_cmd, double &steer_vel_cmd)
{
  ...
  /* 加速度 */ /////
  const double distance = std::hypot(vehicle_status_.pose.position.x - ref_traj_.x[3], vehicle_status_.pose.position.y - ref_traj_.y[3]);
  const double current_v = vehicle_status_.twist.linear.x;
  const double target_v = ref_traj_.vx[3];
  acc_cmd = (target_v * target_v - current_v * current_v) / (2 * distance);



  DEBUG_INFO("[MPC] ref_traj_.vx[0] = %f, ref_traj_.vx[1] = %f, DT = %f", ref_traj_.vx[0], ref_traj_.vx[1], DT);
...
void MPCFollower::callbackVehicleStatus(const nav_msgs::Odometry &msg)
{
  vehicle_status_.tire_angle_rad = msg.twist.twist.angular.z;
  vehicle_status_.twist.linear.x = msg.twist.twist.linear.x;

  // ROS_WARN(vehicle_status_.tire_angle_rad);
  // ROS_WARN(vehicle_status_.twist.linear.x);
  my_steering_ok_ = true;
  my_velocity_ok_ = true;
};
